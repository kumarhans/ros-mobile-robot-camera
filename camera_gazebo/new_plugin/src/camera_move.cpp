...
namespace gazebo
{
  ...
  void GazeboRosPlanarMove::UpdateChild()
  {
    ...
    //ignition::math::Pose3d poseNew(0.0,0.0,2.0,0.0,0.0,0.0);
    ...
    parent_->SetLinearVel(ignition::math::Vector3d(
          ...
          z_));
    parent_->SetAngularVel(ignition::math::Vector3d(rotX_, rotY_, rotZ_));

    //parent_->SetWorldPose(poseNew);
      ...
      const geometry_msgs::Twist::ConstPtr& cmd_msg)
  {
    ...
    rotX_ = cmd_msg->angular.x;
    rotY_ = cmd_msg->angular.y;
    rotZ_ = cmd_msg->angular.z;
  }
  ...
  void GazeboRosPlanarMove::publishOdometry(double step_time)
  {
    ...
    if (rotZ_ > M_PI / step_time)
    {
      // we cannot calculate the angular velocity correctly
      odom_.twist.twist.angular.z = rotZ_;
    }
    else
    {
